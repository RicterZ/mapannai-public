...
import { Marker, CreateMarkerRequest, UpdateMarkerContentRequest, CreateChainRequest, CreateMarkerV2Request } from './types';
...
export class MapannaiApiClient {
  ...
  async createMarker(request: CreateMarkerRequest): Promise<Marker> {
    try {
      const response = await this.client.post('/markers', {
  ...
  async updateMarkerContent(request: UpdateMarkerContentRequest): Promise<Marker> {
    try {
      const response = await this.client.put(`/markers/${request.markerId}`, {
        title: request.title,
  ...
  async getMarkers(): Promise<Marker[]> {
    try {
      const response = await this.client.get('/markers');
      return response.data;
    } catch (error) {
  ...
  async getMarker(markerId: string): Promise<Marker> {
    try {
      const response = await this.client.get(`/markers/${markerId}`);
      return response.data;
    } catch (error) {
      console.error('获取标记失败:', error);
      throw new Error(`获取标记失败: ${error instanceof Error ? error.message : '未知错误'}`);
  ...
  async createChain(request: CreateChainRequest): Promise<{ id: string; name: string; markerIds: string[] }> {
    try {
      const response = await this.client.post('/chains', {
