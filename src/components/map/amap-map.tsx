...
export const AmapMap = () => {
    ...
    // 获取高德地图提供者
    const mapProvider = mapProviderFactory.createProvider('amap')
    const mapConfig: MapProviderConfig = {
        accessToken: config.map.amap.accessToken,
        style: config.map.amap.style,
    }
    ...
    const silentRetryLoad = useCallback(async () => {
        if (loadingRetryCount >= 3 || dataLoaded) return
        ...
        return (
            <div className="w-full h-screen flex items-center justify-center bg-red-50">
                <div className="text-center p-8 bg-white rounded-lg shadow-lg max-w-md">
                    ...
                    <h2 className="text-xl font-semibold text-red-800 mb-2">高德地图加载错误</h2>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="relative">
                                        <input 
                                            type="text" 
                                            value={fabCoordInput} 
                                            onChange={(e) => { setFabCoordInput(e.target.value); if (fabCoordError) setFabCoordError('') }} 
                                            onKeyDown={(e) => { if (e.key === 'Enter') handleFabCoordinateJump() }} 
                                            placeholder="输入坐标，如: 35.452, 139.638" 
                                            className={cn('w-full h-9 pl-9 pr-12 border rounded-md text-sm', fabCoordError ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-blue-500', 'focus:outline-none transition-colors duration-200')} 
                                        />
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="relative">
                                        <input 
                                            type="text" 
                                            value={fabQuery} 
                                            onChange={(e) => { setFabQuery(e.target.value); if (fabQueryError) setFabQueryError(''); if (e.target.value.trim() === '') setFabResults([]) }} 
                                            onKeyDown={(e) => { if (e.key === 'Enter') handleFabSearch() }} 
                                            placeholder="输入关键字，如：东京" 
                                            className={cn('w-full h-9 pl-9 pr-12 border rounded-md text-sm', fabQueryError ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-blue-500', 'focus:outline-none transition-colors duration-200')} 
                                        />
                                        <div className="absolute left-3 top-1/2 -translate-y-1/2">
                                            <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
